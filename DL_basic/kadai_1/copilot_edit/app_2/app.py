...
def load_data():
    """CSVファイルからデータを読み込む"""
    try:
        df = pd.read_csv('tabelog_osaka.csv')
        return df
    except Exception as e:
...
def random_restaurant():
    """大阪府内の飲食店をランダムに表示する"""
    df = load_data()
    osaka_df = df[df['large_area_name'] == '大阪']

    if not osaka_df.empty:
        random_restaurant = osaka_df.sample(n=1).iloc[0]
        restaurant_info = {
            'name': random_restaurant['name'],
            'address': random_restaurant['address'],
            'rating': random_restaurant['rating'],
            'price': random_restaurant['price'],
            'review_count': random_restaurant['review_count'],
            'photo': random_restaurant['photo']
        }
    else:
        restaurant_info = {}

    return render_template('random.html', restaurant=restaurant_info)
